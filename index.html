...
const holidays = [
  ...
  ...Array.from({ length: 14 }, (_, i) => ({
    date: setToMidnight(new Date(2025, 2, 24 + i)), // bulan 2 = Maret
...
const specialWeeks = {
  ...
  today.setHours(0, 0, 0, 0); // Pastikan jam nol untuk perbandingan yang akurat

  // Cek apakah hari ini sebelum atau setelah masa kuliah
  ...
  if (today > pertemuanDates[pertemuanDates.length - 1]) {
    return {
      pertemuan: 17,
      status: "Perkuliahan telah berakhir (Sampai 28 Juli 2025)"
    };
  }

  // Cek apakah hari ini libur nasional
  const hariLibur = holidays.find(h => h.date.getTime() === today.getTime());
  if (hariLibur) {
    return {
      pertemuan: -1,
      status: `Libur Nasional: ${hariLibur.name}`
    };
  }

  // Cek apakah hari ini termasuk minggu khusus (UTS/UAS)
  ...
  // Cek apakah hari ini termasuk salah satu tanggal pertemuan
  const index = pertemuanDates.findIndex(t => t.getTime() === today.getTime());
  if (index !== -1) {
    return {
      pertemuan: index + 1,
      status: `Pertemuan ke-${index + 1}`
    };
  }

  // Jika bukan hari pertemuan dan bukan libur dan bukan UTS/UAS
  return {
    pertemuan: -1,
